  ...
  def main(args: Array[String]): Unit = {
    ...
    val filtered = onTimeDF
      .filter(col("cancelled").equalTo("0.0") && col("origin").isin("CMI", "BWI", "MIA", "LAX", "IAH", "SFO"))
        .select(col("origin"), col("dest"), col("depdelay"))

    val top10Airport = filtered.groupBy("origin","dest")
      .agg(avg("depdelay") as "avgdelay")
      .orderBy($"origin", $"avgdelay".asc)

    top10Airport.write.format("org.apache.spark.sql.cassandra")
      .options(
        Map("table" -> "top10airportsfromx",
          "keyspace" -> "cloudcap"
        )
      ).mode(SaveMode.Overwrite)
      .save()

    //top10Airport.repartition(1).sortWithinPartitions($"Origin", $"AvgDelay".asc)
